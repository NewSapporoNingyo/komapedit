...
| --- | --- |
| Public map ABI | `include/maploader.h`, `include/maploader_snapshot.h`: API v11 / map snapshot v8 functions, fixed-width POD snapshots, Scenario v2 snapshots and direct-save drafts, edit batches, reports, spans, ownership, versions, and structure sizes |
...
- The bundled EXE requires exact maploader API v11, map snapshot v8, and model-loader API v2 matches. `kv_load_map_ex()` is the only map-loading entry point; `KvScenarioSnapshot` v2 is independently allocated and released with `kv_free_scenario_snapshot()`. `kv_save_scenario_document()` accepts a call-scoped `KvScenarioEditDocument`, verifies the expected source hash, reparses the patched Scenario, and transactionally writes the original encoding/BOM/newline. Map, scene, edit-target, and edit-report snapshot versions and structure sizes remain independently versioned.
