...
Each Map parser context owns a distance initialized to zero and its local distance expression. Include seeds copy ordinary variables and source identity, but do not inherit distance; merging preserves the parent's distance and expression while merging child events/control points and variable writes. Speculative Include results are revalidated against ordinary variable dependencies. The file-local distance behavior follows Moboso's parser stack implementation; the official Map page does not explicitly define Include distance scope. The regression contract covers chained/nested/repeated Includes, all load profiles, variable placement arguments, source metadata, and memory Apply/Reset. The original reported map was unavailable; the offset was reproduced and verified using synthetic maps.
...
- `Curve.Interpolate` has no curve row. Its 3D boards come from `Canvas3DSceneRouteInfo::radius_events`, reuse the `CurveCircularStart` visuals, visibility, and picking, keep the edit identity empty so no context action appears, and render `Intpl. 0` for an evaluated zero radius.
