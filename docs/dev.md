...
The visible scene FPS is maintained by `canvas3d_scene_ui.cpp` as a steady-clock interval sample with 0.15 exponential smoothing; gaps longer than 0.25 seconds are ignored so the event-driven canvas keeps its last active value. This overlay value is not Present-completion, refresh-rate, or `--debug-headless-scene3d-bench` `p95_fps` telemetry. Main-loop Present and render wakeups remain owned by `win32_dx11_bootstrap.cpp`.
