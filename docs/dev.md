...
- Store real preferences in `settings/settings.ini`, recent-map/background alignment in `settings/history.ini`, and layout in `settings/imgui.ini`. Settings are marked saved only after the complete file write and close succeed. Failed saves remain pending and retry after one second, including while rendering is idle or occluded.
