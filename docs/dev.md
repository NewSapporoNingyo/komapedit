...
`--debug-headless-sparse-new-element` requires an explicit map path with zero or one numeric distance statement in that target source. It directly drives the production `DrawDistance.Change(500)` wizard form at distance `25`, verifies the target remains selectable, the insertion does not request distance resolution, the canonical tail distance block and typed row are created, then resets and proves the disk hash is unchanged. It is memory-Apply only and rejects `--commit`.
