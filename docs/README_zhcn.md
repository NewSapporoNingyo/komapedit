...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |   √   |    △     |    √     |     △      | 向导可单独新建 Begin/Begin0，或与 End 原子新建；同名区间内的 Begin-only 经确认后作为变化点，成对区间仍拒绝重叠。支持 `repeaterKey` 改名、关联删除/修剪、形式转换和操纵器 |
| `Repeater[repeaterKey].End()`                                                                                                                                                               |   √   |    △     |    √     |     △      | 向导可单独或随 Begin/Begin0 新建 End；允许孤立 End，但拒绝在已有显式 End 的同名有效区间内增加 End。现有 End 距离和关联删除/修剪仍受支持 |
