...
| `Structure[structureKey].Put0(trackKey, tilt, span)`                                                                                                                                        |   √   |    √     |    √     |     △      | 保持 `Put0` 时可通过仅 Z 轴、整米步进的操纵器修改里程；确认转换为 `Put` 后可使用 X/Y/Z 操纵器                   |
...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |   √   |    △     |    ✕     |     △      | 支持关联字段及删除/修剪；未转换的 `Begin0` 可通过仅 Z 轴整米操纵器修改起始里程，转换后可编辑坐标               |
| `Repeater[repeaterKey].End()`                                                                                                                                                               |   √   |    △     |    ✕     |     △      | 支持 End 距离和关联删除/修剪；所属 Begin 的检查器提供仅 Z 轴、整米吸附的 EndDistance 操纵器，但不能通过向导新建 |
