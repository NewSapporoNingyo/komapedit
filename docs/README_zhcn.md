...
- 支持通过 BVE 场景文件（`BveTs Scenario 2.00`）打开地图：按官方 `Route` 条目以场景文件目录为基准解析；存在多个加权候选时通过选择对话框确定。“地图信息列表 -> 其它 -> 场景文件”可只读显示全部八个官方字段，并保留源码中的相对 Route/Vehicle 路径及权重。
...
- `文件 -> 新建...` 打开“新建文件向导”，可新建仅含文件头的 `BveTs Map 2.02` 地图，以及 Structure、Signal、Sound、Sound3D、Station 五类列表文件。资源列表模板提供“导入文件”，可选择已有 `.txt`/`.csv` 并填入仍可编辑的文件名、目录和后缀。向导会将所选的 `.txt`（默认）或 `.csv` 后缀无条件追加到原始文件名，例如输入 `route.csv` 并选择 `.txt` 会生成 `route.csv.txt`；目标已存在普通文件时会复用且绝不写入，否则新建仅含文件头的文件。选择已加载地图作为引用目标时，会在内存工作副本中暂存对应的 `include` 或 `*.Load` 引用；正常“保存”才写入父地图，“撤销”会保留已新建的文件。“预设”分类暂为空，不新增资源列表行或场景文件。
...
| `Station.Load(filePath)`                                                                                                                                                                    |   √   |    △     |    ✕     |     -      | 车站定义行可行内编辑、清空、调整顺序、删除或右键在上下新增行；新增行固定为 13 个 CSV 字段。打开编辑模式后可更换 Load 路径为加载器兼容的 `BveTs Station List`（沿用既有 0.04+ 兼容性） |
...
| `Sound.Load(filePath)`                                                                                                                                                                      |   √   |    △     |    ✕     |     -      | 音效列表行可行内编辑、清空、调整顺序、删除或右键在上下新增行；新增行固定为 3 个 CSV 字段。编辑模式可将 Load 路径更换为加载器兼容的 `BveTs Sound List`（2.00+） |
...
1. 通过菜单 `文件 -> 打开...` 或工具栏 `打开` 选择`.txt`或`.csv`地图文件。选择 BVE 场景文件（`BveTs Scenario 2.00`）时，程序先将其保留为只读文档：“地图信息列表 -> 其它 -> 场景文件”显示全部官方字段、原始相对 Route/Vehicle 路径及权重。单个有效 Route 候选会直接加载地图；多个候选会弹出固定宽度对话框供选择。缺少 Route、Route 目标不存在或取消该对话框时，仍可单独预览场景文件而不加载地图；打开其它地图或场景文件会作为新文档清空该预览，直接打开地图时此菜单项禁用。编辑、保存、重新加载和最近打开历史始终作用于解析出的地图文件；场景权重仅做校验，不用于自动选择。
   - `文件 -> 新建...` 可独立新建只含文件头的地图/列表文件；五种资源列表模板还可导入已有 `.txt`/`.csv`。导入会填入仍可编辑的文件名、路径和后缀；所选后缀始终追加到输入文件名。目标已存在普通文件时会直接复用且不修改内容，目标不存在时才按标准文件头新建。仅地图模板的“确定并载入”会在确认后打开所选地图。若选择已加载地图作为引用目标，则必须启用编辑模式；类型化 `include` 或 `*.Load` 引用只应用到预览，正常“保存”才写盘。
...
6. 在 `辅助信息` 菜单中切换车站、轨道几何、信号、音效、效果和 3D 场景辅助标记。他列车路径通过 `地图信息 -> 他列车列表` 控制。“闭塞标记”默认关闭，并同时控制 2D/3D 中的绿色 `S` 标记及其信号索引参数标签。在 `辅助信息 -> 其它` 中打开 `文件结构图`，可查看入口地图及其嵌套 Include 文件；右键源码文件节点可打开只读文本预览。缺失或无效的 Include 目标会以红色节点保留；其右键菜单禁用文本预览、导入子地图和新建子地图，保留“在文件资源管理器中打开”，以及编辑模式下的“更换文件...”和“解除引用”。即使文件不存在，“在文件资源管理器中打开”仍会打开其目标目录；目录也不存在时会报告错误。启用编辑后，“更换文件...”会将上级地图中的 Include 语句改写为所选 .txt/.csv 子地图（优先相对路径，失败时自动使用绝对路径；预览立即刷新，更改保留到正常保存流程），“解除引用”则经正常的应用/保存流程从上级地图中删除对应 Include 语句；后续语句仍依赖所引用文件时编辑会被阻止。信号标记通过 `地图信号列表` 中的“显示”勾选框控制。
   - “导入子地图...”可选择已有 BVE 地图，并在首个本地距离语句前最后一个 Include 的下方暂存 `include 'path';`；若没有可用 Include，则置于首个距离语句上方；两者皆无时追加到文件末尾。“新建子地图...”通过保存对话框选择目录和文件名，拒绝覆盖已存在文件，使用 UTF-8 无 BOM、CRLF 创建 `BveTs Map 2.02:utf-8`，再走同一暂存 Include 流程。路径优先相对入口地图计算，失败时才使用绝对路径；正常“保存”才会提交父地图。
...
8. 在 `地图信息` 菜单中打开车站、轨道、他列车、布景、连续布景、信号、应答器、音效、轨道变位/粘着、背景、驾驶台亮度、雾效果和绘制距离相关表格。有平面位置的行可定位到平面图；模型和音效文件行可打开关联文件。
   - `信号现示列表`：查看和查找信号现示定义；打开编辑模式后可在主行或 glare 行任意单元格右键，在整个现示/glare 块上方或下方新增固定 6 字段主行。新主行默认无 glare，可手动选择“新增眩光”；已有主行/glare 不会被插入操作拆开，也不提供新增现示列。
   ...
   - 当入口地图及其 Include 中同一种无 key 列表 Load 超过一条，或存在大小写不敏感意义下重复的 `Train[].Enable` 时，本次地图加载会被拒绝
    - `车站列表`：分别查看 `Station.Put` 位置行和 `Station.Load` 定义行；打开编辑模式后，源文件路径的右键菜单可更换已加载的车站列表文件；位置行支持“属性/编辑”和删除，定义行可编辑、调整顺序、清空、删除或右键在上下新增固定 13 字段行
   - `地图布景列表`：查看地图中的 `Structure.Put`、`Structure.Put0`、`Structure.PutBetween`；3D 场景已加载时可定位到对应场景对象
    - `布景模型列表`：查看 `Structure.Load` 指定列表中的 structureKey 和模型文件。车站、布景模型、信号现示、音效文件和 3D 音效文件列表顶部显示加载语句求值后保留原样的路径；悬停可见参数原式与解析绝对路径，右键可在资源管理器中打开。未加载地图或当前地图未指定对应列表时，表格区域会显示“新建或导入文件”入口，并打开匹配的“新建文件向导”模板。每种资源列表在地图中只能引用一份；向导会禁用重复引用，并提示通过顶部“文件路径”的右键“更换文件...”替换。打开编辑模式后，该操作会在内存工作副本中替换对应的 `Station.Load`、`Structure.Load`、`Signal.Load`、`Sound.Load` 或 `Sound3D.Load` 路径，并立即刷新该列表缓存；正常“保存”才写入磁盘。选择的 `.txt`/`.csv` 按地图加载器既有标题/版本规则校验：`BveTs Station List` 沿用既有 0.04+ 兼容规则、`BveTs Structure List` 1.00+、`BveTs Signal Aspects List` 2.00+，以及 Sound 和 Sound3D 共用的 `BveTs Sound List` 2.00+。右键任意文件内容单元格可在上方/下方插入空行（Structure 2 字段、Sound/Sound3D 3、Station 13、Signal 主行 6）。表格内“应用”只提交文件内容行的草稿；目标列表存在未应用草稿或已应用但未保存的旧内容修改时，更换会先确认，并且只丢弃该目标列表的上述修改，保留其它待保存修改。右键 structureKey 并选择 `预览模型` 可打开 3D 模型预览。打开编辑模式后可编辑、调整顺序、清空或删除行
    - 资源列表 CSV 注释：继续支持文档规定的 `#` 注释。为兼容现有 BVE 路线，车站、布景、信号现示、音效和 3D 音效列表行中未被 CSV 双引号包围的 `//` 会在任意位置开始注释；双引号字段内的 `//` 保持字面内容。
    - `连续布景列表`：查看关联的 `Repeater.Begin`/`Begin0`/`End` 段，通过“属性/编辑”修改，并在 3D 场景已加载时定位到对应连续布景实例
      - 连续布景“属性/编辑”中的“插入变化点”会打开对应 Begin/Begin0 的向导，默认仅添加 Begin，两个距离字段填入当前起始里程，并复制当前检查器草稿参数供继续修改；原有确认与“应用/保存”流程保持不变。
          - 打开工具栏“编辑模式”后，可从表格或适用的 2D/3D 标记对已支持的放置、Repeater、闭塞、限速点、轨道变位、应答器、音效/噪声、背景、粘着、驾驶台亮度、雾和绘制距离行使用“属性/编辑”。对于 `Structure.Put`/`Put0` 和 `Repeater.Begin`/`Begin0`，参数输入框上方的按钮可添加或去除全部六项坐标偏移；去除非零偏移时会先确认，因为这些值将被丢弃。使用“新建地图元素”向导新增受支持的地图放置和事件/效果语句；点击“应用并编辑”会在应用成功后关闭向导，并直接打开本次新建主语句的“属性/编辑”窗口（从 Repeater“属性/编辑”窗口打开的向导中该按钮不可用）。Repeater 表单可在一次“应用”中新增 Begin/Begin0、End 或两者；成对新增时 End 里程不得小于 Begin 里程。同名有效区间内的 Begin-only 需确认后作为变化点插入，成对区间仍执行普通重叠保护；已有显式 End 的同名有效区间内禁止增加 End，但允许孤立 End。“轨道几何”分类可新建 `Curve.Begin(radius)`、`Curve.Change(radius)`、`Curve.End()`、`Gradient.Begin(gradient)` 与 `Gradient.End()`；`Curve.Begin(radius, cant)` 始终与前置 `Curve.BeginTransition()` 原子创建，Curve End 和两种 Gradient 操作可选择同时添加缓和曲线起点。每个成对缓和曲线起点和后续 Curve/Gradient 生效语句均可分别设定里程；向导保持 BVE 要求的源语句顺序，且只输出当前官方形式。旧式别名和 Interpolate 不提供新建入口。数值目标 distance 沿用属性/编辑窗口的源码表达式、距离边界和环境检查流程。由 `Station.Load`、`Structure.Load`、`Signal.Load`、`Sound.Load` 和 `Sound3D.Load` 载入的资源行在各自的行内表格中编辑；右键内容单元格可在上方/下方新增空行，Signal 会以整个主行/glare 块定位，新增主行固定 6 字段且默认无 glare，可手动新增 glare。保存前必须先应用各行内表格草稿，否则“保存”会被阻止。“应用”只更新内存预览，工具栏“保存”或 `Ctrl+S` 才写入源文件；“撤销”会丢弃全部待保存的内存改动，“重新加载”会重新读取磁盘文件。
