...
可从工具栏“添加地图元素”打开向导，也可在 3D 场景的“里程选择”模式中右键当前里程打开。先选择目标源文件，再选择模板并填写参数。
...
3. 如需让当前地图引用新文件，在“在以下文件中引用”中选择目标源文件。该操作要求编辑模式。所有已加载的非资源列表地图源文件都是候选；官方 BVE 不要求 `include` 或 `*.Load` 的目标含有距离语句，因此完全空白、无距离语句的地图也会出现在候选中。（单独的“新建地图元素”向导仍要求目标文件含有距离语句。）
