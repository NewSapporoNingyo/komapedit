...
- 支持通过 BVE 场景文件（`BveTs Scenario 2.00`）打开地图：按官方 `Route` 条目以场景文件目录为基准解析；存在多个加权候选时通过选择对话框确定。“地图信息列表 -> 其它 -> 场景文件”显示全部八个官方字段及现有 Route/Vehicle 候选。启用编辑模式后可编辑这些值；场景草稿不经过“应用到内存”，由“保存”直接写入源文件，并保留编码、注释、未知行、重复字段兼容规则、候选顺序及外部修改保护。
...
打开 `BveTs Scenario 2.00` 场景文件时，无论 Route 是否可用，场景文档都会独立保留：

- `地图信息列表 -> 其它 -> 场景文件` 显示场景字段、原始 Route/Vehicle 路径和权重。启用编辑模式后可编辑已有行；右键 Route、Vehicle 或 Image 路径可选择文件或在资源管理器中打开目录。此轮不支持新增、删除或重排候选。
...
- 场景更改通过工具栏“保存”或 `Ctrl+S` 直接写入场景文件，不进入地图 Apply ledger。地图和场景同时有待保存内容时先提交地图；若场景 Route 路径已改变，会在控制台和状态栏警告并延后场景写入，下一次保存可单独写入。权重只用于校验，不用于自动选择 Route。
- “最近打开的地图”、背景对齐历史、“重新加载”和“仅重新加载地图几何”均以场景文件路径为文档入口；重新加载会重新解析场景，需要时再次显示候选选择框并刷新模型预览；仅重新加载地图几何时会尽量保留已有场景模型和相机。直接打开地图文件仍保持原有地图入口行为。
...
- **场景文件**：只在打开 Scenario 文档时可用，显示全部官方字段及现有候选。编辑模式关闭时只读，启用后可编辑；场景“保存”会直接写入源文件。
