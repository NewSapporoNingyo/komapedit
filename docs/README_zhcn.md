...
| `Structure.Load(filePath)`                                                                                                                                                                  |   √   |    △     |    ✕     |     -      | 已加载列表中的已有 key/path 可行内编辑、清空、调整顺序或删除；编辑模式可将 Load 路径更换为加载器兼容的 `BveTs Structure List`（1.00+）；不能新增行 |
...
| `Station.Load(filePath)`                                                                                                                                                                    |   √   |    △     |    ✕     |     -      | 已有车站定义行可行内编辑、清空、调整顺序或删除；打开编辑模式后可更换 Load 路径为加载器兼容的 `BveTs Station List`（沿用既有 0.04+ 兼容性）；不能新增行 |
...
| `Signal.Load(filePath)`                                                                                                                                                                     |   √   |    △     |    ✕     |     -      | 已有现示/glare 行可行内编辑、清空、调整顺序或删除；编辑模式可将 Load 路径更换为加载器兼容的 `BveTs Signal Aspects List`（2.00+）；不支持新行/列及显示超过 509 个 structure-key 列 |
...
| `Sound.Load(filePath)`                                                                                                                                                                      |   √   |    △     |    ✕     |     -      | 已有音效列表行可行内编辑、清空、调整顺序或删除；编辑模式可将 Load 路径更换为加载器兼容的 `BveTs Sound List`（2.00+）；不支持新增行 |
| `Sound[soundKey].Play()`                                                                                                                                                                    |   √   |    √     |    √     |     ✕      | 距离/key 支持编辑、新建和删除；不实际播放音频                                                                  |
| `Sound3D.Load(filePath)`                                                                                                                                                                    |   √   |    △     |    ✕     |     -      | 已有 3D 音效列表行可行内编辑、清空、调整顺序或删除；编辑模式可将 Load 路径更换为加载器兼容的 `BveTs Sound List`（2.00+）；不支持新增行 |
