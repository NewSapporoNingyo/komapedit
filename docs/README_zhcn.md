...
- `文件 -> 新建...` 打开“新建文件向导”，可新建仅含文件头的 `BveTs Map 2.02` 地图，以及 Structure、Signal、Sound、Sound3D、Station 五类列表文件。向导会将所选的 `.txt`（默认）或 `.csv` 后缀无条件追加到原始文件名，例如输入 `route.csv` 并选择 `.txt` 会生成 `route.csv.txt`，且不会覆盖已有文件。地图模板另有“确定并载入”，成功创建后会自动打开新地图；若当前地图有未保存更改，则复用既有的重新加载确认，取消后保留当前地图及新建文件。选择已加载地图作为引用目标时，会在内存工作副本中暂存对应的 `include` 或 `*.Load` 引用；正常“保存”才写入父地图，“撤销”不会删除已新建的文件。“预设”分类暂为空，不新增资源列表行或场景文件。
...
1. 通过菜单 `文件 -> 打开...` 或工具栏 `打开` 选择`.txt`或`.csv`地图文件。选择 BVE 场景文件（`BveTs Scenario 2.00`）时，程序按其 `Route` 条目解析地图入口：单候选直接加载，多候选时弹出固定宽度对话框列出原始相对路径供选择。编辑、保存、重新加载和最近打开历史始终作用于解析出的地图文件；场景权重仅做校验，不用于自动选择。
   - `文件 -> 新建...` 可独立新建只含文件头的地图/列表文件，所选 `.txt`（默认）或 `.csv` 后缀始终追加到输入文件名。仅地图模板的“确定并载入”会在成功创建后打开新地图；有未保存更改时，既有的重新加载确认可取消载入且不会删除新文件。若选择已加载地图作为引用目标，则必须启用编辑模式；类型化 `include` 或 `*.Load` 引用只应用到预览，正常“保存”才写盘，“撤销”不会删除已经新建的文件。
