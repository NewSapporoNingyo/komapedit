...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |   √   |    △     |    ✕     |     △      | 支持关联字段及删除/修剪；`Begin0` 需确认转换后才能编辑坐标，显式关联 End 则具有独立的距离操纵器                 |
