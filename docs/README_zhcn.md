...
| `Light.Ambient(...)`、`Light.Diffuse(...)`、`Light.Direction(...)`                                                                                                                          |   √   |    ✕     |    ✕     |     ✕      | 只读“光照效果”标签页显示求值后的数值。RGB 必须在 `[0, 1]`；Direction 必须位于里程 `0`；根地图及全部 Include 中同类语句重复时，该类全部无效并报告所有源码位置。不实现编辑、新建、标记或 3D 光照模拟 |
...
- Route 缺失、目标不存在或取消选择时，仍可查看场景文件，但不会加载地图。
