...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |   √   |    △     |    ✕     |     △      | 支持关联字段及删除/修剪；检查器按钮可双向转换，`Begin0` 使用仅 Z 轴整米操纵器修改起始里程                       |
