...
- [x] 在编辑模式下编辑现有场景字段及 Route/Vehicle 候选路径和权重。场景草稿不进入地图 Apply ledger，由“保存”直接写入；保存校验源哈希、仅补丁最后生效字段、按需追加缺失标量、拒绝候选增删/重排并完整重解析，同时保留原编码/BOM/换行；Route 路径变化在重新加载场景前持续显示警告。
