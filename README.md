...
| `Curve.BeginTransition()`                                                                                                                                                                   |    √    |       △       |      △      |         ✕         | The wizard creates it only atomically before a two-argument `Curve.Begin` or selected `Curve.End`, with an independently set start distance; no standalone transition template |
| `Curve.Begin(radius, cant)` / `[legacy] Curve.BeginCircular(radius, cant)`                                                                                                                  |    √    |       √       |      △      |         ✕         | The wizard emits current `Curve.Begin(radius, cant)` only atomically with a preceding transition at its own distance; `Curve.BeginCircular` remains edit-only              |
...
| `Curve.End()`                                                                                                                                                                               |    √    |       √       |      √      |         ✕         | The wizard inserts End, optionally atomically with a transition start at its own distance; existing rows remain editable and deletable                                      |
...
| `Gradient.BeginTransition()`                                                                                                                                                                |    √    |       △       |      △      |         ✕         | The wizard creates it only atomically before a selected `Gradient.Begin` or `Gradient.End`, with an independently set start distance; no standalone transition template    |
| `Gradient.Begin(gradient)` / `[legacy] Gradient.BeginConst(gradient)`                                                                                                                       |    √    |       √       |      √      |         ✕         | The wizard emits current `Gradient.Begin(gradient)`, optionally atomically with a preceding transition at its own distance; `Gradient.BeginConst` remains edit-only       |
| `Gradient.End()`                                                                                                                                                                            |    √    |       √       |      √      |         ✕         | The wizard inserts End, optionally atomically with a transition start at its own distance; existing rows remain editable and deletable                                      |
