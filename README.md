...
| `include 'file';`                                                                                                                                                                           |    √    |       △       |      ✕      |         -         | Nested Include files participate in parsing and supported contained elements can be written back; with Edit enabled, an Include reference can be removed from the File Structure Diagram context menu, and removal is blocked when surviving statements still depend on that file |
...
6. Use the `Auxiliary Info` menu to toggle plan marker groups for stations, track geometry, signals, sounds, effects, and 3D scene helpers. Other-train paths are controlled from the `Other Train List` in `Map Info`. `Section Markers` is off by default and controls the green `S` markers and their signal-index parameter labels in both 2D and 3D. Under `Auxiliary Info -> Other`, open `File Structure Diagram` to inspect the entry map and its nested Include files; right-click a source-file node to open its read-only text preview, or choose `Unlink Include` with Edit enabled to remove that Include statement from its parent map through the normal Apply/Save workflow. Signal markers are controlled from the `Map Signal List` row `Show` checkboxes in `Map Info`.
