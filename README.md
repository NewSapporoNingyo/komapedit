...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |    √    |       △       |      ✕      |         △         | Linked fields and deletion/trim are supported; `Begin0` needs confirmed conversion for coordinate editing, while an explicit linked End exposes its own distance gizmo          |
