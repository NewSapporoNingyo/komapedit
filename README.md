...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |    √    |       △       |      ✕      |         △         | `repeaterKey` renames update every Begin/Begin0/End in the linked chain; overlapping same-name intervals are rejected. Linked deletion/trim, form conversion, and gizmos are supported |
