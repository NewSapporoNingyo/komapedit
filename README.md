...
| `Structure[structureKey].Put0(trackKey, tilt, span)`                                                                                                                                        |    √    |       √       |      √      |         △         | Basic fields can be edited or created; keeping `Put0` exposes a Z-only whole-metre distance gizmo, while confirmed conversion to `Put` enables X/Y/Z                          |
...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |    √    |       △       |      ✕      |         △         | Linked fields and deletion/trim are supported; unconverted `Begin0` has a Z-only whole-metre begin-distance gizmo, while conversion enables coordinate editing                 |
