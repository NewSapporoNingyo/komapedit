...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |    √    |       △       |      ✕      |         △         | Linked fields and deletion/trim are supported; Inspector buttons convert both ways, and `Begin0` uses a Z-only whole-metre begin-distance gizmo                                |
| `Repeater[repeaterKey].End()`                                                                                                                                                               |    √    |       △       |      ✕      |         △         | End distance and linked deletion/trim are supported; the owning Begin Inspector exposes a Z-only whole-metre EndDistance gizmo, but wizard insertion is unavailable             |
