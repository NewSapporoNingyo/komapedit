...
| `Repeater[repeaterKey].Begin(trackKey, x, y, z, rx, ry, rz, tilt, span, interval, structureKey1, ...)` / `Repeater[repeaterKey].Begin0(trackKey, tilt, span, interval, structureKey1, ...)` |    √    |       △       |      √      |         △         | The wizard creates both official forms with a required structure-key list; canonical same-name interval overlaps are rejected. `repeaterKey` renames, linked deletion/trim, form conversion, and gizmos are supported |
| `Repeater[repeaterKey].End()`                                                                                                                                                               |    √    |       △       |      ✕      |         △         | Renamed with its owning chain; End distance and linked deletion/trim are supported. The owning Begin Inspector exposes the EndDistance gizmo, but wizard insertion is unavailable |
