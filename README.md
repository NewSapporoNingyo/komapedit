...
| `Track[trackKey].X.Interpolate(x, radius)` / `Track[trackKey].X.Interpolate(x)` / `Track[trackKey].X.Interpolate()`                                                                         |    √    |       △       |      ✕      |         ✕         | All official forms feed other-track geometry; distance/numeric fields and deletion are supported. `trackKey` stays read-only in Properties/Edit but can be renamed for the whole track from Other Tracks |
