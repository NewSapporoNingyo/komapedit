...
- Evaluated-event markers that must edit their source should bind during hydration by stable source identity, never by mileage or a distance-sorted event's occurrence order. `Curve.Interpolate` pairs typed Curve rows and shared radius events through the existing `KvElementRow` source file and statement global order. Preview must leave unavailable provenance unset, and the Edit metadata merge must propagate verified row indices into both Plan and Scene caches; both views then share edit identity, picking, highlight, and context actions.
