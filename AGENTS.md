...
- Registered non-headless CTest contracts are `multilanguage_contract`, `typed_snapshot_contract`, `maploader_gradient_projection_contract`, `typed_edit_contract`, `maploader_diagnostics_contract`, `canvas3d_camera_contract`, and `route_value_sampling_contract`. Headless validation must be invoked explicitly and must not be registered with CTest.
