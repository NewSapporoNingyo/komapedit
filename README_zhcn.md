...
- [x] 支持旋转和缩放布景模型预览
- [x] 3D 画布场景预览，可显示轨道路径、布景、连续布景、信号、背景变化和插值后的 BVE 雾效果
...
11. 在 `3D 视图 -> 3D场景预览` 中显示场景预览窗口，然后点击 `启动3D场景预览`。场景预览可在窗口中重新加载或关闭；加载场景后，车站跳转和里程跳转也会移动场景相机。叠加层会显示当前曲线/超高、坡度和下一站信息。`选项 -> 3D画布设置 -> 雾效果` 可即时切换场景预览中的线路雾效果，且默认开启。选择模式下可从场景对象定位回布景或连续布景表格；打开编辑模式后，右键布景并选择“属性/编辑”可修改字段，受支持的 `Structure.Put` 坐标也可使用 X/Y/Z 操纵器拖动。
