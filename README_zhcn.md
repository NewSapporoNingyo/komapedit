...
- [ ] 信号和应答器列表编辑
- [x] 为已支持的布景/`Station.Put` 表格行和 3D 场景布景对象提供基于源锚点的“属性/编辑”检查器
- [ ] 将属性检查器扩展到 2D 标记、其余地图信息行和 3D 场景连续布景对象
...
- [x] 支持旋转和缩放布景模型预览
- [x] 3D 画布场景预览，可显示轨道路径、布景、连续布景、信号和背景变化
- [x] 可通过车站跳转和数字里程跳转移动 3D 场景相机，并在平面图上显示当前 3D 位置
- [x] 可从布景/连续布景表格定位到 3D 场景对象，也可从 3D 场景对象定位回对应表格
- [ ] 3D 场景画质设置：render scale、MSAA、纹理过滤和轮廓质量
- [x] 在 3D 场景线路信息叠加层显示当前曲线半径/超高、坡度和距下一站距离
- [ ] 为 3D 线路信息叠加层补充上一站信息和当前不支持的插值情况
- [x] 通过实时 3D 操纵器编辑 `Structure.Put` 的 X/Y/Z 位置，并支持可选的 `Put0` 转换和操纵器尺寸设置
- [ ] 支持通过 3D 操纵器编辑布景旋转和其他放置字段
- [ ] 连续布景编辑后同步重建场景中由该 repeater 生成的实例
...
- [x] 预览缓存管理：显示文件数量和占用空间、手动清除，并可持久化禁用缓存
- [x] Include 文件结构图，以及读取当前内存工作副本的只读源码文本预览
- [x] 编辑模式中分离“应用到预览”“保存到磁盘”“撤销”和“从磁盘重新加载”，并在存在未保存更改时确认
- [x] 将自轨道和他轨道几何导出为 CSV
...
磁盘预览缓存会优先保存到可执行文件旁的 `cache\preview-v2`。如果该目录
不可写，则使用 `%LOCALAPPDATA%\komapedit\cache\preview-v2`，不会再退回系统
临时目录。`选项 -> 缓存管理...` 只显示和管理本次启动选中的活动缓存位置，
不会迁移或删除非活动候选位置中的缓存。禁用缓存后，后续地图加载不会读取或
写入磁盘预览缓存；已有缓存文件会保留，直至用户手动清除。
...
8. 在 `地图信息` 菜单中打开车站、轨道、他列车、布景、连续布景、信号、应答器、音效、轨道变位/粘着、背景、驾驶台亮度和雾效果相关表格。有平面位置的行可定位到平面图；模型和音效文件行可打开关联文件。
   - `信号现示列表`：查看信号现示定义
   ...
   - `连续布景列表`：查看 `Repeater.Begin/End`；3D 场景已加载时可定位到对应连续布景实例
   - 打开工具栏“编辑模式”后，可对已支持的 `Station.Put`、布景模型列表和布景放置行使用“属性/编辑”。“应用”只更新内存预览，工具栏“保存”才写入源文件；“重新加载”会重新读取磁盘文件。
9. 在 `2D 视图 -> 背景图` 中导入背景图，可手动调整位置、尺寸、旋转和亮度，也可按两个车站对齐
10. 在 `3D 视图 -> 布景模型预览` 中显示或隐藏布景模型预览窗口。预览窗口内可用鼠标左键拖动旋转模型，用鼠标滚轮缩放
11. 在 `3D 视图 -> 3D场景预览` 中显示场景预览窗口，然后点击 `启动3D场景预览`。场景预览可在窗口中重新加载或关闭；加载场景后，车站跳转和里程跳转也会移动场景相机。叠加层会显示当前曲线/超高、坡度和下一站信息。选择模式下可从场景对象定位回布景或连续布景表格；打开编辑模式后，右键布景并选择“属性/编辑”可修改字段，受支持的 `Structure.Put` 坐标也可使用 X/Y/Z 操纵器拖动。
12. 在 `选项 -> 缓存管理...` 中查看活动缓存目录、文件数和总大小，可打开目录、确认后清除全部预览缓存，或禁用后续地图加载的缓存读写
13. 在 `文件 -> 导出 CSV...` 中选择输出目录，导出自轨道和他轨道几何 CSV
14. 按 `F5` 或菜单 `文件 -> 重新加载` 可重新读取当前地图
