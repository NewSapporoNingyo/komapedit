...
当前项目已经支持布景模型列表、`Structure.Put`/`Put0`/`PutBetween`、`Station.Put` 和相互关联的 `Repeater.Begin`/`Begin0`/`End` 段的源文件关联编辑，并可在 3D 场景中实时拖动布景和 Repeater Begin 的 X/Y/Z 位置；但尚不是完整的地图编辑器，自轨道曲线/坡度、声音、环境效果和新建元素等编辑仍在开发计划中。
...
- [ ] 信号和应答器列表编辑
- [x] 为已支持的布景、`Station.Put` 和关联 Repeater 行提供基于源锚点的“属性/编辑”检查器，并在 3D 场景中为可编辑的布景和 Repeater Begin 放置提供实时 X/Y/Z 操纵器
...
| 数学函数                                      |   √   |    ✕     |     -      | 用于地图表达式求值                                                                   |
| 距离声明及距离表达式                          |   √   |    △     |     ✕      | 仅 `Structure.Put/Put0/PutBetween`、`Station.Put` 和关联 Repeater Begin/End 等编辑目标可修改距离 |
...
8. 在 `地图信息` 菜单中打开车站、轨道、他列车、布景、连续布景、信号、应答器、音效、轨道变位/粘着、背景、驾驶台亮度、雾效果和绘制距离相关表格。有平面位置的行可定位到平面图；模型和音效文件行可打开关联文件。
   ...
   - `布景模型列表`：查看 `Structure.Load` 指定列表中的 structureKey 和模型文件；右键 structureKey 并选择 `预览模型` 可打开 3D 模型预览
    ...
    - 打开工具栏“编辑模式”后，可对已支持的 `Station.Put`、布景模型/放置和关联连续布景行使用“属性/编辑”。“应用”只更新内存预览，工具栏“保存”才写入源文件；“撤销”会丢弃全部待保存的内存改动，“重新加载”会重新读取磁盘文件。
...
11. 在 `3D 视图 -> 3D场景预览` 中显示场景预览窗口，然后点击 `启动3D场景预览`。场景预览可在窗口中重新加载或关闭；加载场景后，车站跳转和里程跳转也会移动场景相机。叠加层会显示当前曲线/超高、坡度和下一站信息。`选项 -> 3D画布设置 -> 雾效果` 可即时切换场景预览中的线路雾效果，且默认开启；同一设置还可控制地图语句驱动的绘制距离、相机速度和场景实例性能警告。选择模式下可从场景对象和支持的地图元素标记定位回对应表格；打开编辑模式后，右键布景或连续布景并选择“属性/编辑”可修改字段，受支持的 `Structure.Put` 和 `Repeater.Begin` 坐标也可使用 X/Y/Z 操纵器拖动。
