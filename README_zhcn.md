...
5. 在 `2D 视图` 菜单中切换站点、站名、里程、曲线、限速、坡度图、曲线半径图，以及应答器、背景变化点、驾驶台亮度变化点和雾效果变化点等标记层。信号标记通过 `地图信号列表` 中的“显示”勾选框控制。
6. 在 `模式` 中选择“测量”，在轨道附近移动或双击，可查看里程、标高、坡度、曲线半径和限速
7. 在 `地图信息` 菜单中打开信号、应答器、音效、背景、驾驶台亮度和雾效果相关表格。信号、应答器、背景、驾驶台亮度和雾效果行可定位到平面图；音效行则可打开关联文件。
   - `信号现示列表`：查看信号现示定义
   - `地图信号列表`：查看信号位置，并使用行内“显示”勾选框控制平面图标记
   - `应答器列表`：查看应答器位置
   - `音效文件列表` 和 `3D音效文件列表`：查看已加载的音效文件条目、查找匹配 key、查找未使用条目，并打开关联文件
   - `地图音效列表` 和 `地图3D音效列表`：查看音效播放/固定音源位置，并定位到平面图
   - `背景变化点列表`、`驾驶台亮度变化点列表` 和 `雾效果变化点列表`：查看对应变化点表
