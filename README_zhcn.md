...
- [ ] 为可编辑 map/list 语句建立源锚点和稳定编辑 metadata，同时保持 `kv_get_ir_json()` 兼容
- [ ] 在源 map/include/list 文件中编辑、删除、插入并保存元素，尽量保留 include 结构、距离表达式、原始编码和换行
...
- [ ] 信号和应答器列表编辑
- [ ] 为 Map Info 行、2D 标记和 3D 场景对象提供基于源锚点的统一属性检查器
...
- [ ] 3D 相机信息补全：当前曲线半径、坡度、距上一站/下一站距离
- [ ] 3D 画布中的布景编辑功能
...
6. 在 `辅助信息` 菜单中切换车站、轨道几何、他列车路径、信号、音效、效果和 3D 场景辅助标记。在 `辅助信息 -> 其它` 中打开 `文件结构图`，可查看入口地图及其嵌套 Include 文件。信号标记通过 `地图信号列表` 中的“显示”勾选框控制。
...
11. 在 `3D 视图 -> 3D场景预览` 中显示场景预览窗口，然后点击 `启动3D场景预览`。场景预览可在窗口中重新加载或关闭；加载场景后，车站跳转和里程跳转也会移动场景相机。选择模式下可从场景对象定位回布景或连续布景表格。
